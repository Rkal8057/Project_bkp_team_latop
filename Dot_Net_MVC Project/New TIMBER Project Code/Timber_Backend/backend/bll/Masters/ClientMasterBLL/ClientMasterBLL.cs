...
namespace RTA.Masters
{
    public class ClientMasterBLL
    {
        ...
        public ClientMasterResp GetClientMaster( string? flagName )
        {
            ClientMasterResp data = new ClientMasterResp()
            {
                status = true,
                Message = "data fetched successfully",
                ClientMasterData = DB.ClientMaster.Where(e => e.client_status == 0 && e.irflag == flagName).OrderBy(e=> e.position).ToList()
            };
            return data;
        }
        ...
        public ClientMasterResp UpdateClientMaster(long clientId, ClientMasterReq samp)
        {
            ...
            existingClient.irflag = samp.irflag;
            existingClient.position = samp.position;
            DB.SaveChanges();
