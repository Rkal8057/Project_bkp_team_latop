...
public static class DatabaseSeeder
{
    public static void SeedData<T>(loginDBContext context, string jsonFilePath) where T : class, new()
    {
       ...
       // Perform left join to find records not in the database
        if(jsonFilePath.Contains("state_master"))
        {
            var dataToAdd = newData.Except(existingData, new GenericComparer<T>("State_Id"));
        // Add missing data to the database
        dbSet.AddRange(dataToAdd);
        context.SaveChanges();

        }else if(jsonFilePath.Contains("vou_list_master")){
             var dataToAdd = newData.Except(existingData, new GenericComparer<T>("vou_type_id"));
        // Add missing data to the database
        dbSet.AddRange(dataToAdd);
        context.SaveChanges();
        }
