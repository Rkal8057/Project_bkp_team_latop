    ...
    public class ItemMasterBLL
    {
        ...
        public ItemMasterResp InsertItemMaster(ItemMasterReq samp)
        {
            ...
            ItemMaster SampData = new ItemMaster()
            {
                ...
                warranty_yes_no = samp.warranty_yes_no,
        ...
        public ItemMasterResp GetAllItemMaster(int pageNo, int limit, string searchKey, long? partyId)
        {

            IQueryable<ItemMaster> query = DB.ItemMaster
                 .Include(aggre => aggre.ChallanTransDetail.Where( e=> e.delete_at == 0 && e.irflag =="IC" 
                 && e.pcode == partyId))
        ...
        public ItemMasterResp ItemNameExistsInDatabase(ItemMasterReq samp)
        {
            ...
            {
                status = false,
                Message = $"Item name '{samp.item_name.Trim()}' already exists.",
        ...
        public ItemMasterResp UpdateItemMaster(long itemId, ItemMasterReq samp)
        {
            ...
            if( existingItem != null )
            {
                if( existingItem.item_name !=  samp.item_name )
                {
                    ItemMaster checkName = DB.ItemMaster.Where( e=> e.item_name == samp.item_name ).FirstOrDefault();   
                    if( checkName != null )
                    {
            ...
            existingItem.item_short_name = samp.item_short_name;
            existingItem.item_name = samp.item_name;
            existingItem.group = samp.group;
            existingItem.group_id = samp.group_id;
            ...
            existingItem.warranty_period = samp.warranty_period;
            existingItem.warranty_yes_no = samp.warranty_yes_no;
            existingItem.item_date = samp.item_date;
        ...
        public ItemMasterResp CheckDeleteItemMasterIfExitInMaster(long itemId, string Masterflag)
        {
            ...
            if (Masterflag == "itemCategory")
            {
                CategoryMaster category= DB.CategoryMaster.Where( e=> e.id  == itemId).FirstOrDefault();

                var itemMaster = DB.ItemMaster
                 .Where(e => e.group == category.category_name && e.delete_at == 0)
                 .ToList();
                ...
                if (itemMaster.Count != 0)
                {
                    ...
                    {
                        status = false,
                        Message = "Item Category cannot be deleted because it exists in:"
                    };


                    if (itemMaster != null)
                    {
                        data1.Message += " Item Detail";
                        data1.ItemMasterDetailData = itemMaster;
                    }
            ...
            if (Masterflag == "voucher")
            {
                var voucher = DB.vou_master
                  .Include(e => e.vou_trn_master)
                  .Where(E => E.id == itemId).FirstOrDefault();
                ...
                if (voucher != null)
                {
                    ...
                    {
                        status = false,
                        Message = "Voucher can be deleted"
                    };

                    data1.VoucherMaster = voucher;
        ...
        public ItemMasterResp DeleteItemMaster(
            string Masterflag, long Id, long? otp = null)
        {
           ...
           /* if ( otp != 1234)
            ...
            if (Masterflag == "issue_challan" || Masterflag == "issue_recipted" || Masterflag == "issue_sale" || Masterflag == "issue_purchase")
            {
            ...
            if(Masterflag == "voucher")
            {
                ...
                if (del.vou_trn_master != null)
                {
                    foreach (var e in del.vou_trn_master.ToList())
                    {
                        e.status = 1;
                    }
                }
            ...
            if (Masterflag == "itemCategory")
            {
                ...
                CategoryMaster del = DB.CategoryMaster.SingleOrDefault(parent => parent.id == Id);
                if (del == null)
                {
                    throw new Exception("RECORD NOT FOUND");
                }
